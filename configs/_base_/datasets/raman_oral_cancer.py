...
train_pipeline = [
    ...
    # dict(type='RemoveBaseline', roi=[[-29, 4090]], method='als', lam=10 ** 5, p=0.05),
...
data = dict(
    samples_per_gpu=256,  # single GPU  Batch size
    workers_per_gpu=2,  #  GPU the Thread count
    train=dict(
        type=dataset_type,
        data_size=(0, 0.7),
        file_path='data/oral_cancer/results/oral_cancer.csv',

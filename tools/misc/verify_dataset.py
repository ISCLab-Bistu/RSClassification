...
from rmsm import Config, DictAction, track_parallel_progress, track_progress

from rmsm.datasets import PIPELINES, build_dataset
...
class DatasetValidator():
    ...
    def __init__(self, dataset_cfg, log_file_path, phase):
        ...
        # keep only LoadDataFromFile pipeline
        assert dataset_cfg.data[phase].pipeline[0][
            'type'] == 'LoadDataFromFile', 'This tool is only for dataset ' \
            'that needs to load data from files.'
    ...
    def valid_idx(self, idx):
        ...
        try:
            ...
            with open(self.log_file_path, 'a') as f:
                ...
                filepath = os.path.join(item['data_prefix'],
                                        item['ram_info']['filename'])
                f.write(filepath + '\n')

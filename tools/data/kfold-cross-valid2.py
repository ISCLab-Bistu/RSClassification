...
#     # dict(type='Smoothing', method="GCVSmoothedNSpline", window_length=5, polyorder=2),
#     # dict(type='Smoothing', method="MSESmoothedNSpline", window_length=5, polyorder=2),
#     # dict(type='Smoothing', method="DOFSmoothedNSpline", window_length=5, polyorder=2),
#     dict(type='Smoothing', method="whittaker", window_length=5, polyorder=2),
#     dict(type='Smoothing', method="interpolation", window_length=5, polyorder=2),
#     dict(type='Smoothing', method="likelihood", window_length=5, polyorder=2),
#     dict(type='Smoothing', method="flat", window_length=5, polyorder=2),
# ]
#
# # baseline correction
# baseline_correction = [
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='poly', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='iter_poly', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='exp', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='als', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='airPLS', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='arPLS', lam=10 ** 5, p=0.05),
#     dict(type='RemoveBaseline', roi=[[0, 2089]], method='drPLS', lam=10 ** 5, p=0.05),
# ]
#
# # Normalization
# normalization_list = [
#     dict(type='Normalize', method='area'),
...
if __name__ == '__main__':
    # config_path = "../../configs/resnet50/raman_single_cell.py"
    # cfg = Config.fromfile(config_path)  # The corresponding configuration file is obtained after parsing
    # cfg.config_path = config_path
    # for i in range(len(smoothing_noise)):
    #     cfg.work_dir = "smoothing_single_cell"
    #     cfg.data.train.pipeline[1] = smoothing_noise[i]
    #     cfg.data.val.pipeline[1] = smoothing_noise[i]
    #     cfg.data.test.pipeline[1] = smoothing_noise[i]
    #     cfg.search_train = smoothing_noise[i]['method']
    #     main(cfg)
    #
    # config_path = "../../configs/resnet50/raman_single_cell.py"
    # cfg = Config.fromfile(config_path)  # The corresponding configuration file is obtained after parsing
    # cfg.config_path = config_path
    # for i in range(len(baseline_correction)):
    #     cfg.work_dir = "baseline_single_cell"
    #     cfg.data.train.pipeline[2] = baseline_correction[i]
    #     cfg.data.val.pipeline[2] = baseline_correction[i]
    #     cfg.data.test.pipeline[2] = baseline_correction[i]
    #     cfg.search_train = baseline_correction[i]['method']
    #     main(cfg)
    #
    # config_path = "../../configs/resnet50/raman_single_cell.py"
    # cfg = Config.fromfile(config_path)  # The corresponding configuration file is obtained after parsing
    ...
    #     cfg.work_dir = "normalization_single_cell"
    #     cfg.data.train.pipeline[3] = normalization_list[i]
    #     cfg.data.val.pipeline[3] = normalization_list[i]
    #     cfg.data.test.pipeline[3] = normalization_list[i]
    #     cfg.search_train = normalization_list[i]['method']
    #     main(cfg)

    # main("../../configs/transformer/raman_pnas_dataset.py")
    # main("../../configs/mobilenet/raman_cell_spectrum.py")
    main("../../configs/transformer/raman_cell_spectrum.py")
    # main("../../configs/resnet/raman_ovarian_cancer_move.py")
    # main("../../configs/resnet/raman_ovarian_cancer_intensity.py")
    # main("../../configs/resnet/raman_ovarian_cancer_gan.py")

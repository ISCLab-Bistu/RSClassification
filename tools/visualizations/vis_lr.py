...
from rmsm import Config, DictAction, ProgressBar
from rmsm.runner import (EpochBasedRunner, IterBasedRunner, IterLoader,
                         build_optimizer)
from torch.utils.data import DataLoader

from rmsm.utils import get_root_logger
...
def plot_curve(lr_list, cfg, iters_per_epoch, by_epoch=True):
    """Plot learning rate vs iter graph."""
    cfg.style = 'whitegrid'
    cfg.window_size = '12*7'
    try:
        ...
        print("Attention: The plot style won't be applied because 'seaborn' "
              ...
              'show style.')
    wind_w, wind_h = cfg.window_size.split('*')
    ...
    if cfg.get('title') is None:
        plt.title(f'{osp.basename(path)} Learning Rate curve')
    else:
        plt.title(cfg.title)

    if cfg.save_path:
        plt.savefig(cfg.save_path + "/lr_config")
        print(f'The learning rate graph is saved at {cfg.save_path}')
    plt.show()
...
def main(path):
    ...
    # prepare data loader
    cfg.ngpus = 1
    batch_size = cfg.data.samples_per_gpu * cfg.ngpus

    if cfg.get('dataset_size') is None and by_epoch:
        from rmsm.datasets.builder import build_dataset
        dataset_size = len(build_dataset(cfg.data.train))
    else:
        dataset_size = cfg.dataset_size or batch_size
    ...
    plot_curve(lr_list, cfg, len(data_loader), by_epoch)

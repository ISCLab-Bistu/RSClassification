...
import numpy as np
from rmsm.runner import HOOKS, BaseRunner
from rmsm.runner.dist_utils import master_only
from rmsm.runner.hooks.checkpoint import CheckpointHook
from rmsm.runner.hooks.evaluation import DistEvalHook, EvalHook
from rmsm.runner.hooks.logger.wandb import WandbLoggerHook


@HOOKS.register_module()
class RmsmWandbHook(WandbLoggerHook):
    """Enhanced Wandb logger hook for classification.

    Comparing with the :cls:`rmsm.runner.WandbLoggerHook`, this hook can not
    ...
    - **Checkpoints**: If ``log_checkpoint`` is True, the checkpoint saved at
      ...
      the : class:`rmsm.runner.CheckpointHook` whose priority is higher than
    ...
    - **Evaluation**: At every interval, this hook logs the model prediction as
      ...
      ``num_eval_data``. Currently, this hook logs the predicted labels along
    ...
    Args:
        ...
        log_checkpoint_metadata (bool): Log the evaluation metrics computed
            ...
            Defaults to True.
        num_eval_datas (int): The number of validation datas to be logged.
    ...
    def __init__(self,
                 ...
                 num_eval_datas=100,
                 **kwargs):
        super(RmsmWandbHook, self).__init__(init_kwargs, interval, **kwargs)

        self.log_checkpoint = log_checkpoint
        self.log_checkpoint_metadata = (
            log_checkpoint and log_checkpoint_metadata)
        self.num_eval_datas = num_eval_datas
        self.log_evaluation = (num_eval_datas > 0)
        self.ckpt_hook: CheckpointHook = None
        self.eval_hook: EvalHook = None

    @master_only
    def before_run(self, runner: BaseRunner):
        super(RmsmWandbHook, self).before_run(runner)
        ...
        if self.log_checkpoint:
            if self.ckpt_hook is None:
                ...
                runner.logger.warning(
                    'To log checkpoint in rmsmWandbHook, `CheckpointHook` is'
        ...
        if self.log_evaluation or self.log_checkpoint_metadata:
            if self.eval_hook is None:
                ...
                runner.logger.warning(
                    ...
                    'rmsmWandbHook, `EvalHook` or `DistEvalHook` in rmsm '
            ...
            if (self.log_evaluation
                    and self.num_eval_datas > len(self.val_dataset)):
                self.num_eval_datas = len(self.val_dataset)
                runner.logger.warning(
                    f'The num_eval_datas ({self.num_eval_datas}) is '
        ...
        if self.log_checkpoint_metadata:
            assert self.ckpt_interval % self.eval_interval == 0, \
                'To log checkpoint metadata in rmsmWandbHook, the interval ' \
    ...
    def after_train_epoch(self, runner):
        super(RmsmWandbHook, self).after_train_epoch(runner)
    ...
    def after_train_iter(self, runner):
        if self.get_mode(runner) == 'train':
            ...
            return super(RmsmWandbHook, self).after_train_iter(runner)
        else:
            super(RmsmWandbHook, self).after_train_iter(runner)
    ...
    def _init_data_table(self):
        ...
        columns = ['data_name', 'data', 'ground_truth']
        self.data_table = self.wandb.Table(columns=columns)

    def _init_pred_table(self):
        """Initialize the W&B Tables for model evaluation."""
        columns = ['epoch'] if self.by_epoch else ['iter']
        columns += ['data_name', 'data', 'ground_truth', 'prediction'
                    ] + list(self.val_dataset.CLASSES)
        self.eval_table = self.wandb.Table(columns=columns)

    def _add_ground_truth(self):
        # Get data loading pipeline
        from rmsm.datasets.pipelines import LoadDataFromFile
        spectrum_loader = None
        for t in self.val_dataset.pipeline.transforms:
            if isinstance(t, LoadDataFromFile):
                spectrum_loader = t

        CLASSES = self.val_dataset.CLASSES
        self.eval_data_indexs = np.arange(len(self.val_dataset))
        # Set seed so that same validation set is logged each time.
        np.random.seed(42)
        np.random.shuffle(self.eval_data_indexs)
        self.eval_data_indexs = self.eval_data_indexs[:self.num_eval_datas]

        for idx in self.eval_data_indexs:
            spectrum_info = self.val_dataset.data_infos[idx]
            if spectrum_loader is not None:
                spectrum_info = spectrum_loader(spectrum_info)
                # Get data and convert from BGR to RGB
                data = spectrum_info['spectrum'][..., ::-1]
            else:
                # For CIFAR dataset.
                data = spectrum_info['spectrum']
            data_name = spectrum_info.get('filename', f'spectrum_{idx}')
            labels = spectrum_info.get('labels').item()

            self.data_table.add_data(data_name, self.wandb.Image(data),
                                     CLASSES[labels])

    def _add_predictions(self, results, idx):
        table_idxs = self.data_table_ref.get_index()
        assert len(table_idxs) == len(self.eval_data_indexs)

        for ndx, eval_data_index in enumerate(self.eval_data_indexs):
            result = results[eval_data_index]

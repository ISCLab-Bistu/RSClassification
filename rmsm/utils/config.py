...
class Config:
    ...
    Example:
        ...
        >>> cfg.filename
        "/home/kchen/projects/rmsm/tests/data/config/a.py"
        >>> cfg.item4
        'test'
        >>> cfg
        "Config [path: /home/kchen/projects/rmsm/tests/data/config/a.py]: "
    ...
    def _file2dict(filename, use_predefined_variables=True):
        ...
        with tempfile.TemporaryDirectory() as temp_config_dir:
            ...
            if filename.endswith('.py'):
                ...
                import rmsm
                cfg_dict = rmsm.utils.load(temp_config_file.name)
            # close temp file
            temp_config_file.close()
    ...
    def dump(self, file=None):
        ...
        if file is None:
            if self.filename is None or self.filename.endswith('.py'):
                ...
                return rmsm.utils.dump(cfg_dict, file_format=file_format)
        elif file.endswith('.py'):
            with open(file, 'w', encoding='utf-8') as f:
                f.write(self.pretty_text)
        else:
            file_format = file.split('.')[-1]
            return rmsm.utils.dump(cfg_dict, file=file, file_format=file_format)
    ...
    def merge_from_dict(self, options, allow_list_keys=True):
        ...
        Examples:
            ...
            >>> options = dict(pipeline={'0': dict(type='SelfLoadData')})
            >>> cfg.merge_from_dict(options, allow_list_keys=True)
            >>> cfg_dict = super(Config, self).__getattribute__('_cfg_dict')
            >>> assert cfg_dict == dict(pipeline=[
            ...     dict(type='SelfLoadData')])

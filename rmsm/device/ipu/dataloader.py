...
from rmsm.parallel import DataContainer
...
def collate(batch, samples_per_gpu=1):
    ...
    TODO support for
    :type:`~rmsm.parallel.DataContainer`. Currently, it will be ignored.
    There are 3 cases.

    1. cpu_only = True, e.g., meta data.
    2. cpu_only = False, stack = True, e.g., data tensors.
    """
...
class IPUDataLoader(torch.DataLoader):
    ...
    Args:
        ...
        options (torch.Options): Options that will be used to compile
            and run the model.
        ...
        mode (torch.DataLoaderMode, optional): If `DataLoaderMode.Async`,
            uses an :py:class:`~torch.AsynchronousDataAccessor` to access
            the dataset. If `DataLoaderMode.Sync`, accesses the dataset
            synchronously.
        async_options (Dict[str, Any], optional): Options to pass to
            :py:class:`~torch.AsynchronousDataAccessor`.
    ...
    def init(self, options, **kwargs):
        if not self.initialized:
            ...
            if kwargs['mode'] == 'sync':
                kwargs['mode'] = torch.DataLoaderMode.Sync
            elif kwargs['mode'] == 'async':
                kwargs['mode'] = torch.DataLoaderMode.AsyncRebatched

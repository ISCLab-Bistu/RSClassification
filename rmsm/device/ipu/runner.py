...
from rmsm.runner import (HOOKS, RUNNERS, BaseRunner, EpochBasedRunner,
                         IterBasedRunner)
...
class IPUBaseRunner(BaseRunner):
    ...
    Args:
        ...
        options_cfg (rmsm.Config, dict): Options that will be used to compile
            and run the model.
        modules_to_record (rmsm.Config, list): Index or name of modules which
            will be recorded for output. It is necessary to specify output for
            static graph of model training or inference.
        ipu_model_cfg (rmsm.Config, dict): Config of model partition and
            recomputing checkpoint
        fp16_cfg (rmsm.Config): Config for fp16 training.

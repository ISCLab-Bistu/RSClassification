...
from rmsm.cnn import build_conv_layer, build_norm_layer
from rmsm.cnn.bricks.transformer import AdaptivePadding
from rmsm.runner.base_module import BaseModule
...
def resize_relative_position_bias_table(src_shape, dst_shape, table, num_head):
    ...
    for i in range(num_head):
        ...
        f_cubic = interpolate.interp1d(x, y, z, kind='cubic')
...
class PatchEmbed(BaseModule):
    """Raman Spectrum to Patch Embedding.

    We use a conv layer to implement PatchEmbed.

    Args:
        ram_size (int | tuple): The size of input raman spectrum. Default: 224
    ...
    def __init__(self,
        ...
        warnings.warn('The `PatchEmbed` in rmsm will be deprecated. '
                      'Please use `rmsm.cnn.bricks.transformer.PatchEmbed`. '
                      "It's more general and supports dynamic input shape")

        if isinstance(ram_size, int):
            ram_size = to_2tuple(ram_size)
        elif isinstance(ram_size, tuple):
            if len(ram_size) == 1:
                ram_size = to_2tuple(ram_size[0])
            assert len(ram_size) == 2, \
                f'The size of data should have length 1 or 2, ' \
                f'but got {len(ram_size)}'
        ...
        _conv_cfg = dict(
            type='Conv1d', kernel_size=16, stride=16, padding=0, dilation=1)
        _conv_cfg.update(conv_cfg)
        self.projection = build_conv_layer(_conv_cfg, in_channels, embed_dims)

        # Calculate how many patches a input data is splited to.
        h_out, w_out = [(self.ram_size[i] + 2 * self.projection.padding[i] -
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H == self.ram_size[0] and W == self.ram_size[1], \
            f"Input data size ({H}*{W}) doesn't " \
            f'match model ({self.ram_size[0]}*{self.ram_size[1]}).'
...
# Modified from pytorch-data-models
class HybridEmbed(BaseModule):
    ...
    Args:
        ...
        ram_size (int | tuple): The size of input data. Default: 224
    ...
    def __init__(self,
        ...
        if feature_size is None:
            with torch.no_grad():
                ...
                if training:
                    backbone.eval()
                o = self.backbone(
                    torch.zeros(1, in_channels, ram_size[0], ram_size[1]))
        ...
        conv_cfg = conv_cfg or dict()
        _conv_cfg = dict(
            type='Conv1d', kernel_size=1, stride=1, padding=0, dilation=1)
        _conv_cfg.update(conv_cfg)
...
class PatchMerging(BaseModule):
    """Merge patch feature map. Modified from rmsm, which uses pre-norm layer

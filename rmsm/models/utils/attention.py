...
from rmsm.cnn.bricks.registry import DROPOUT_LAYERS
from rmsm.cnn.bricks.transformer import build_dropout
from rmsm.cnn.utils.weight_init import trunc_normal_
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        B = query.shape
        H = hw_shape
        assert B == H, f"The query length doesn't match the input "\
            f'shape ({H}).'
        query = query.view(B, H)

        window_size = self.window_size
        shift_size = self.shift_size

        if min(H) == window_size:
            ...
            assert self.pad_small_map, \
                f'The input shape ({H}) is smaller than the window ' \
    ...
    def get_attn_mask(hw_shape, window_size, shift_size, device=None):
        if shift_size > 0:
            ram_mask = torch.zeros(1, *hw_shape, 1, device=device)

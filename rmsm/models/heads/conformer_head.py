...
class ConformerHead(ClsHead):
    ...
    def forward_train(self, x, labels):
        ...
        losses = self.loss([conv_cls_score, tran_cls_score], labels)
        return losses
    ...
    def loss(self, cls_score, labels):
        ...
        # compute loss
        loss = sum([
            self.compute_loss(score, labels, avg_factor=num_samples) /
            len(cls_score) for score in cls_score
        ])
        if self.cal_acc:
            # compute accuracy
            acc = self.compute_accuracy(cls_score[0] + cls_score[1], labels)

...
import rmsm
from rmsm.parallel.scatter_gather import scatter
from rmsm.parallel.utils import is_module_wrapper
...
class PaviLoggerHook(LoggerHook):
    ...
    Args:
        ...
        add_graph (bool, optional): **Deprecated**. Whether to visual model.
            Default: False.
        ram_key (str, optional): **Deprecated**. Data key. Defaults to None.
        ...
        add_graph_kwargs (dict, optional): A dict contains the params for
            ...
            - ram_key (str): Get data data from Dataset. Default: 'ram'.
            - opset_version (int): ``opset_version`` of exporting onnx.
                Default: 11.
            - dummy_forward_kwargs (dict, optional): Set default parameters to
                model forward function except data. For example, you can set
                {'return_loss': False} for rmsm. Default: None.
    ...
    def __init__(self,
        ...
        self.ram_key = add_graph_kwargs.get('ram_key', 'ram')
        ...
        if ram_key is not None:
            warnings.warn(
                '"ram_key" is deprecated in `PaviLoggerHook`, please use '
                'the key "ram_key" of add_graph_kwargs instead',
                DeprecationWarning)
    ...
    def before_run(self, runner) -> None:
        ...
        if runner.meta is not None:
            if 'config_dict' in runner.meta:
                ...
                config_dict = dict(rmsm.Config.fromfile(config_file))
            else:
                config_dict = None
            if config_dict is not None:
                ...
                # rmsm.utils.dump to json
                config_dict = json.loads(
                    rmsm.utils.dump(config_dict, file_format='json'))
    ...
    def _add_graph(self, runner, step: int) -> None:
        from rmsm.runner.iter_based_runner import IterLoader

...
from rmsm.utils.fileio import load as load_file
from rmsm.parallel import is_module_wrapper
from rmsm.utils import digit_version, load_url, mkdir_or_exist
from .dist_utils import get_dist_info

ENV_rmsm_HOME = 'rmsm_HOME'
ENV_XDG_CACHE_HOME = 'XDG_CACHE_HOME'
DEFAULT_CACHE_DIR = '~/.cache'


def _get_rmsm_home() -> str:
    rmsm_home = os.path.expanduser(
        os.getenv(
            ENV_rmsm_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'rmsm')))

    mkdir_or_exist(rmsm_home)
    return rmsm_home
...
def get_torchvision_models():
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(rmsm.__path__[0],
                             'model_zoo/torchvision_0.12.json')
        model_urls = rmsm.load(json_path)
...
def get_external_models():
    rmsm_home = _get_rmsm_home()
    default_json_path = osp.join(rmsm.__path__[0], 'model_zoo/open_mmlab.json')
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(rmsm_home, 'open_mmlab.json')
...
def get_rmsm_models():
    rmsm_json_path = osp.join(rmsm.__path__[0], 'model_zoo/rmsm.json')
    rmsm_urls = load_file(rmsm_json_path)

    return rmsm_urls


def get_deprecated_model_names():
    deprecate_json_path = osp.join(rmsm.__path__[0],
...
def _process_rmsm_checkpoint(checkpoint: Dict) -> Dict:
...
def load_from_ceph(filename: str,
    ...
    .. warning::
        :class:`rmsm.utils.fileio.file_client.CephBackend` will be deprecated,
        please use :class:`rmsm.utils.fileio.file_client.PetrelBackend` instead.
...
def load_from_openmmlab(
    ...
    if model_url.startswith(('http://', 'https://')):
        ...
        filename = osp.join(_get_rmsm_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f'{filename} can not be found.')
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
...
@CheckpointLoader.register_scheme(prefixes='rmsm://')
def load_from_rmsm(
    filename: str,
    map_location: Union[str, Callable, None] = None,
) -> Union[dict, OrderedDict]:
    """load checkpoint through the file path prefixed with rmsm.

    Args:
        filename (str): checkpoint file path with rmsm prefix
        map_location (str, optional): Same as :func:`torch.load`.
    ...
    model_urls = get_rmsm_models()
    model_name = filename[8:]
    checkpoint = load_from_http(
        model_urls[model_name], map_location=map_location)
    checkpoint = _process_rmsm_checkpoint(checkpoint)
    return checkpoint
...
def save_checkpoint(model: torch.nn.Module,
    ...
    meta.update(rmsm_version=rmsm.__version__, time=time.asctime())

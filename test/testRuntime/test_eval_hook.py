...
from rmsm.runner import obj_from_dict
from rmsm.runner.hooks import DistEvalHook, EvalHook
from torch.utils.data import DataLoader, Dataset

from rmsm.apis import single_gpu_test
...
def test_iter_eval_hook():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner = rmsm.runner.IterBasedRunner(
            model=model,
            optimizer=optimizer,
            work_dir=tmpdir,
...
def test_epoch_eval_hook():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner = rmsm.runner.EpochBasedRunner(
            model=model,
            optimizer=optimizer,
            work_dir=tmpdir,
            logger=logging.getLogger(),
...
@patch('rmsm.apis.multi_gpu_test', multi_gpu_test)
def test_dist_eval_hook():
    ...
    # test DistEvalHook
    with tempfile.TemporaryDirectory() as tmpdir:
        p = patch('rmsm.engine.multi_gpu_test', multi_gpu_test)
        p.start()
